...
public class ProductForm {
	...
	private void initialize(boolean addOrSuggest) {
		...
		/*
		 * productName.addActionListener(new ActionListener() { public void
		 * actionPerformed(ActionEvent arg0) {
		 * if(productName.getSelectedItem()!="") {
		 * Categories.setSelectedItem(products.get(productName.getSelectedIndex(
		 * )-1).getCategory());
		 * Brands.setSelectedItem(products.get(productName.getSelectedIndex()-1)
		 * .getBrand()); } } });
		 */
		...
		Categories.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (Categories.getSelectedIndex() != 0) {
					...
					for (Product p : storeGUI.getStoreUIController().GetSystemProducts()) {
						if (p.getCategory().equals( Categories.getSelectedItem()))
							productName.addItem(p.getName());
					}
				} else {
		...
		Categories.addItem("All categories");
		...
		btnAddProduct.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (txtQuantity.getText().isEmpty()) {
					...
					storeGUI.getStoreUIController().AddProduct(productName.getSelectedItem().toString(),
							Float.parseFloat(txtPrice.getText()), Integer.parseInt(txtQuantity.getText()),
							Categories.getSelectedItem().toString(), Brands.getSelectedItem().toString());
		...
		btnSugestProduct.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (SuggestedPName.getText().isEmpty()) {
					...
					storeGUI.getStoreUIController().SuggestProduct(SuggestedPName.getText(),
							Categories.getSelectedItem().toString(), Brands.getSelectedItem().toString());
